...
const fetchGoogleBooksDetails = async (googleBooksId) => {
  try {
    ...
    const isbn = data.volumeInfo.industryIdentifiers
...
export const addBookToUser = async (req, res) => {
  ...
  try {
    ...
    if (!result) {
      return res
        .status(500)
        .json({ error: "Failed to add book to the user's library." });
    }
...
export const getFavoriteGenreAndAuthor = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({
      favoriteAuthors: topAuthors.map((author) => author.author) || [
        "Not available",
      ],
      mostReadGenres: topGenres.map((genre) => genre.genre) || [
        "Not available",
      ],
    });
  } catch (error) {
...
export const toggleFavorite = async (req, res) => {
  ...
  try {
    ...
    if (!userBook) {
      return res
        .status(404)
        .json({ error: "Book not found in user's bookshelf" });
    }
