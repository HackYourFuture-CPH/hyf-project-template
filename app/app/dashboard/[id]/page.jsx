    ...
    const fetchUserData = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_APP_API_URL}/users/profile/${id}`,
          {
    ...
    <AppLayoutContainer>
      <div className={styles.mainContent}>
        ...
        <div className={styles.middleContent}>
          ...
          <MostRecentQuote quotes={userData.recentQuotes} />{" "}
          {/* Pass recent quotes */}
          <UserProgress progress={userData.progress} />{" "}
          {/* Pass user progress */}
        </div>

        <div className={styles.rightSide}>
          <h3>Favorite Quotes:</h3>
          <Quote favoriteQuotes={userData.favoriteQuotes} />{" "}
          {/* Pass favorite quotes */}
        </div>
      </div>
    </AppLayoutContainer>
  );
